...
data_final <- total_data %>%
  ...
  mutate(variables = ifelse(variables == "san_francisco_violent", "San Francisco", variables),
         ...
         variables = ifelse(variables == "baltimore_violent", "Baltimore", variables),
         variables = ifelse(variables == "chicago_violent", "Chicago", variables),
         variables = ifelse(variables == "st_louis_violent", "St. Louis", variables)) %>%
  accumulate_by(~year)

# Saves data into RDS file.

saveRDS(object = data_final, file = "RDS/imprisonment.RDS")
```
